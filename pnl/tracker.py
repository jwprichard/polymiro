...
# Ensure the project root is importable when run as ``python -m pnl.tracker``.
_PROJECT_ROOT = Path(__file__).resolve().parent.parent
if str(_PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(_PROJECT_ROOT))

import config
from pnl.gamma_client import check_resolution
from pnl.calculator import calculate_pnl, calculate_exit_pnl
...
_DRY_TRADES_FILE: Path = config.DATA_DIR / "dry_trades.json"
_UPDOWN_TRADES_FILE: Path = config.DATA_DIR / "updown_trades.json"
...
def _load_json_list(path: Path) -> list[dict]:
    ...
    if not path.exists():
        logger.info("Trade file not found, treating as empty: %s", path)
        return []
    try:
        ...
        if not isinstance(data, list):
            logger.warning("Expected JSON array in %s, got %s — skipping", path, type(data).__name__)
            return []
        return data
...
def run() -> None:
    ...
    if not dry_only:
        logger.info("No dry-mode trades to process — exiting.")
        return
    ...
    for trade in gamma_candidates:
        ...
        if not market_id:
            logger.warning("Trade %s has no market_id — skipping", trade.get("trade_id"))
            continue
    ...
    logger.info("Wrote %d new P&L records to %s", len(new_results), _PNL_REPORT_FILE)
